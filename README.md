...
它不需要账号、没有云同步、没有远程数据库，也不会把词表、学习进度或导入文件发送到任何第三方。学习数据只保存在这台电脑上。
...
- **一个词本，一套进度**：可以新建、切换、归档或删除本地单词本；切换词本后学习、复习、错词库、日历记录都会同步切换。
