...
- **统一离线英语声音（可选）**：在“设置”中主动准备后，电脑、iPhone 与 iPad 都使用同一份免费开源英语声音；模型只保存在对应设备的浏览器私有空间，朗读的单词不会上传或同步。
...
| 统一离线英语声音（可选） | 用户主动准备后约占 110 MB（量化 Kokoro 模型约 92 MB），保存在各设备浏览器私有空间；采用 HeadTTS、Kokoro 与 CMU 英语发音词典，不上传朗读内容，也不参与同步。 |
