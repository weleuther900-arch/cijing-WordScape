...
function markLearningSeen(word) {
  ...
  const batch = getBatch(); const batchWords = batch ? dailyStudyWordsForBatch(batch) : []; const learned = batchWords.filter((card) => card.learningSeen).length;
  const summary = document.querySelector("[data-learning-summary]"); if (summary) summary.innerHTML = `已学习 <strong>${learned}</strong> / ${batchWords.length}`;
  const dailyProgress = document.querySelector(".daily-progress-line"); if (dailyProgress) dailyProgress.innerHTML = `今日已学习 <strong>${learned} / ${batchWords.length}</strong> 个单词`;
}
