...
function renderLearn() {
  ...
  if (!plannedWords.length) {
    ...
    const copy = gateClosed ? `到期复习已有 ${overdueReviewCount()} 词；完成一部分后会自动恢复新词安排。` : noReleasedWords ? "导入的单词将在已核对的例句准备完成后加入每日计划。" : "当前词本中的新词已安排完毕。";
    const action = gateClosed ? `<button class="primary" data-go="review">去复习</button>` : noReleasedWords ? `<button class="primary" data-go="words">去导入词表</button>` : `<button class="secondary" data-go="words">查看词表</button>`;
